...
/**
 * This example is similar to "basic-example", the only difference is that it shows how to dump data to a file. It
 ...
 * - Run a mem leak test in verbose mode and dump data to a file. This will output all places in the code where memory
 * was allocated and not yet freed. It will also summarize the total number of leaked objects as well as the amount of
 * memory leaked
...
int main(int argc, const char* argv[])
{
	...
	// stop collecting mem allocations info and clean the reminaing mem leaks. This will also be dumped to
	// "mem_allocations.log"
