...
/**
 * This example shows how to enable MemPlumber to collect information about allocation of static variables. It
 * demonstrates the following flow:
 * - Allocate some static variables (1 static variable and 2 static members of a class)
 * - Run a static mem test in verbose mode. This will output all places in the code where static memory was allocated.
 * It will also summarize the total number of static objects as well as the amount of memory in those objects
 * - When enabling static memory collection you should rename your `main` method and instead add the `MEMPLUMBER_MAIN`
 * macro and give it your alternative (renamed) main method
 *
...
// in order to collect static memory allocation we need to rename our `main` method and add the `MEMPLUMBER_MAIN` with
// the alternative main method as a parameter
